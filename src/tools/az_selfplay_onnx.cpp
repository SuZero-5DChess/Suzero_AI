...
constexpr int kDefaultPieceChannels = 12;
...
// Free function to find board index by (l, t, c) — used by both CaptureKingEnv
// and SemimoveMcts for halfstep-level action entry construction.
[[nodiscard]] int find_board_index(
...
struct GameResult {
    ...
    struct TrainingSample {
        ...
        int player = 0;
        // Pending_from coordinates for destination nodes.
        // Source nodes use -1 sentinel (pending_l = -1).
        int pending_x = -1;
        int pending_y = -1;
        int pending_t = -1;
        int pending_l = -1;
...
constexpr uint32_t kDataVersion = 5U;
...
public:
    ...
    void write_game(const GameResult& game) {
        ...
        for (const auto& sample : game.samples) {
            ...
            // pending_from coordinates (4 int32, -1 sentinel for source nodes)
            write_pod(out_, static_cast<int32_t>(sample.pending_x));
            write_pod(out_, static_cast<int32_t>(sample.pending_y));
            write_pod(out_, static_cast<int32_t>(sample.pending_t));
            write_pod(out_, static_cast<int32_t>(sample.pending_l));
...
public:
        ...
        auto process_lines = [&](const std::vector<int>& lines) {
            for (int line : lines) {
                ...
                for (int pos : marked_pos(pieces)) {
                    ...
                    for (const vec4& to : state_ref().gen_piece_move(from, c)) {
    ...
    [[nodiscard]] EncodedState encode_state(std::optional<Semimove> pending_from = std::nullopt) const {
        ...
        encoded.current_player = current_player();
        ...
        // Mark pending_from in board_planes so the network can distinguish
        // source-selection vs destination-selection nodes.
        if (pending_from.has_value()) {
            const Semimove& pf = *pending_from;
            const int b_idx = find_board_index(encoded.board_keys, pf, current_player());
            if (b_idx >= 0) {
                const int sq = pf.from.x() + pf.from.y() * board_side_;
                if (0 <= sq && sq < board_squares_) {
                    for (int ch = 0; ch < kDefaultPieceChannels; ++ch) {
                        const size_t idx = static_cast<size_t>(b_idx) * kDefaultPieceChannels * board_squares_
                                         + static_cast<size_t>(ch) * board_squares_
                                         + static_cast<size_t>(sq);
                        encoded.board_planes[idx] *= -1.0f;
                    }
                }
            }
        }
...
public:
    ...
    [[nodiscard]] std::vector<std::pair<float, std::vector<float>>> predict_actions_batch(
        ...
        std::vector<int64_t> side_to_move_values(batch_size, 0);
        ...
        for (size_t batch_idx = 0; batch_idx < active_batch; ++batch_idx) {
            ...
            side_to_move_values[batch_idx] = request.side_to_move;
            if (num_boards > 0) {
                std::copy(
        ...
        const std::array<int64_t, 1> side_to_move_shape = {static_cast<int64_t>(batch_size)};
        ...
        std::array<Ort::Value, 13> input_tensors = {
            ...
            Ort::Value::CreateTensor<int64_t>(memory_info_, side_to_move_values.data(), side_to_move_values.size(), side_to_move_shape.data(), side_to_move_shape.size()),
...
class MCTSNode {
public:
    explicit MCTSNode(MCTSNode* parent = nullptr, ActionChoice action = {}, std::optional<Semimove> pending_from = std::nullopt)
        : parent(parent), action(action), pending_from(pending_from) {}
    ...
    void expand(const std::vector<std::pair<ActionChoice, float>>& child_specs, bool terminal, float terminal_score,
                std::optional<Semimove> child_pending_from = std::nullopt) {
        is_expanded = true;
        ...
        for (const auto& [child_action, child_prior] : child_specs) {
            auto child = std::make_unique<MCTSNode>(this, child_action, child_pending_from);
    ...
    ActionChoice action{};
    std::optional<Semimove> pending_from;
    float prior = 0.0f;
    int visit_count = 0;
...
public:
    ...
    [[nodiscard]] SearchResult select_action(CaptureKingEnv& env, float urgency, float temperature,
                                                     std::optional<Semimove> pending_from = std::nullopt) {
        const auto select_start = std::chrono::steady_clock::now();
        transposition_table_.clear();
        MCTSNode root;
        root.pending_from = pending_from;
        std::vector<ActionEntry> root_entries;
        (void)expand_node(root, env, urgency, &root_entries, pending_from);
        ...
        while (sim < cfg_.num_simulations) {
            ...
            while (sim < cfg_.num_simulations && static_cast<int>(pending.size()) < std::max(1, cfg_.leaf_batch_size)) {
                ...
                while (node->is_expanded && !node->is_terminal && !node->children.empty()) {
                    MCTSNode* child = node->select_child(cfg_.c_puct);
                    path.push_back(child);

                    if (child->action.is_submit) {
                        ...
                        if (outcome.has_value()) {
                            ...
                            child->terminal_value = white_to_current_player_value(*outcome, scratch.current_player());
                        }
                    } else if (!child->pending_from.has_value()) {
                        // Destination → Source transition: complete the semimove
                        const auto apply_start = std::chrono::steady_clock::now();
                        if (!scratch.apply_semimove(child->action.semimove, false)) {
                    ...
                    // Source → Destination transition: no env change (pending_from set)
                    node = child;
                }
                if (profile_ != nullptr) {
...
private:
    ...
    void expand_pending_leaves_batched(std::vector<PendingLeaf>& pending, float urgency) {
        ...
        for (size_t i = 0; i < pending.size(); ++i) {
            ...
            const bool is_dest = node.pending_from.has_value();

            // Build TT key including pending_from
            std::string tt_key = leaf.env.transposition_key()
                + (is_dest ? "|pf=" + std::to_string(node.pending_from->from.x()) + ","
                                    + std::to_string(node.pending_from->from.y()) + ","
                                    + std::to_string(node.pending_from->from.t()) + ","
                                    + std::to_string(node.pending_from->from.l())
                           : "|pf=-");
            ...
            if (leaf.env.done()) {
                ...
                if (cfg_.use_transposition_table) {
                    transposition_table_[tt_key] = TranspositionEntry{
                        terminal_value, true, terminal_value, node.pending_from, {}, {}
                    };
                }
                continue;
            }
            ...
            if (is_dest) {
                // ── Destination node: filter to legal destinations for pending_from ──
                const Semimove& pf = *node.pending_from;
                std::vector<Semimove> dests;
                for (const auto& sm : legal_semimoves) {
                    if (sm.from.l() == pf.from.l() &&
                        sm.from.x() == pf.from.x() && sm.from.y() == pf.from.y() &&
                        sm.from.t() == pf.from.t()) {
                        dests.push_back(sm);
                    }
                }
                if (dests.empty()) {
                    ...
                    resolved_values[i] = 0.0f;
                    if (cfg_.use_transposition_table) {
                        transposition_table_[tt_key] = TranspositionEntry{
                            0.0f, true, 0.0f, node.pending_from, {}, {}
                        };
                    }
                    continue;
                }
                ...
                EncodedState encoded = leaf.env.encode_state(node.pending_from);
                if (profile_ != nullptr) {
                    profile_->encode_state.add(seconds_since(encode_start));
                }
                const auto build_start = std::chrono::steady_clock::now();
                std::vector<ActionEntry> entries = build_action_entries(leaf.env, encoded, dests, false, node.pending_from);
                if (profile_ != nullptr) {
                    profile_->build_action_entries.add(seconds_since(build_start));
                }
                batch_requests.push_back(OnnxPolicyValue::BatchRequest{
                    std::move(encoded), std::move(entries), urgency, leaf.env.current_player()
                });
            ...
            // Deduplicate source coordinates
            std::vector<Semimove> unique_sources;
            {
                std::set<std::tuple<int, int, int, int>> seen;
                for (const auto& sm : legal_semimoves) {
                    auto key = std::make_tuple(sm.from.l(), sm.from.x(), sm.from.y(), sm.from.t());
                    if (seen.insert(key).second) {
                        unique_sources.push_back(sm);
                    }
                }
            }
            ...
            entries.reserve(unique_sources.size() + (can_submit ? 1 : 0));
            const int player = leaf.env.current_player();
            for (const auto& sm : unique_sources) {
                ActionEntry entry;
                entry.action = ActionChoice{false, sm};
                entry.board_idx = static_cast<int64_t>(find_board_index(encoded.board_keys, sm, player));
        ...
        if (!batch_requests.empty()) {
            ...
            for (size_t req_idx = 0; req_idx < batch_requests.size(); ++req_idx) {
                ...
                const bool is_dest = node.pending_from.has_value();
                if (is_dest) {
                    // Destination node children: source nodes (pending_from=nullopt)
                    node.expand(child_specs, false, 0.0f, std::nullopt);
                } else {
                    // Source node children: destination nodes (pending_from=source)
                    node.expand(child_specs, false, 0.0f, std::nullopt);
                    for (auto& child : node.children) {
                        if (!child->action.is_submit) {
                            child->pending_from = child->action.semimove;
                        }
                    }
                }
    ...
    [[nodiscard]] std::vector<ActionEntry> build_action_entries(
        ...
        const EncodedState& encoded,
        const std::vector<Semimove>& legal_halfsteps,
        bool can_submit,
        std::optional<Semimove> pending_from = std::nullopt
    ) const {
        std::vector<ActionEntry> entries;
        entries.reserve(legal_halfsteps.size() + (can_submit ? 1 : 0));
        const int player = env.current_player();
        if (pending_from.has_value()) {
            // ── Destination node: score at each legal DESTINATION square ──
            const Semimove& pf = *pending_from;
            Semimove full_sm = pf;  // copy pending_from source fields
            for (const auto& half : legal_halfsteps) {
                full_sm.to = half.to;  // set destination to the actual destination
                ActionEntry entry;
                entry.action = ActionChoice{false, full_sm};
                // Score at the DESTINATION square (half.to = destination coord)
                Semimove dest_only = half;
                dest_only.from = half.to;  // for find_board_index: use destination coord
                entry.board_idx = static_cast<int64_t>(find_board_index(encoded.board_keys, dest_only, player));
                entry.from_sq = static_cast<int64_t>(half.to.x() + half.to.y() * cfg_.board_side);
                entry.is_submit = 0;
                entries.push_back(entry);
            }
        } else {
            // ── Source node: score at each legal SOURCE square + submit ──
            for (const auto& sm : legal_halfsteps) {
                ActionEntry entry;
                entry.action = ActionChoice{false, sm};
                entry.board_idx = static_cast<int64_t>(find_board_index(encoded.board_keys, sm, player));
                entry.from_sq = static_cast<int64_t>(sm.from.x() + sm.from.y() * cfg_.board_side);
    ...
    [[nodiscard]] float expand_node(
        ...
        const bool is_dest = pending_from.has_value();

        // Build transposition key including pending_from
        std::string tt_key = env.transposition_key()
            + (is_dest ? "|pf=" + std::to_string(pending_from->from.x()) + ","
                                + std::to_string(pending_from->from.y()) + ","
                                + std::to_string(pending_from->from.t()) + ","
                                + std::to_string(pending_from->from.l())
                       : "|pf=-");
        if (cfg_.use_transposition_table) {
            const auto tt_lookup_start = std::chrono::steady_clock::now();
            auto found = transposition_table_.find(tt_key);
        ...
        if (env.done()) {
            ...
            if (cfg_.use_transposition_table) {
                transposition_table_[tt_key] = TranspositionEntry{
                    terminal_value, true, terminal_value, pending_from, {}, {}
                };
            }
            if (profile_ != nullptr) {
                profile_->expand_node.add(seconds_since(expand_start));
        ...
        if (is_dest) {
            ...
            const Semimove& pf = *pending_from;
            std::vector<Semimove> dests;
            dests.reserve(legal_semimoves.size());
            for (const auto& sm : legal_semimoves) {
                if (sm.from.l() == pf.from.l() &&
                    sm.from.x() == pf.from.x() && sm.from.y() == pf.from.y() &&
                    sm.from.t() == pf.from.t()) {
            ...
            if (dests.empty()) {
                const float terminal_value = 0.0f;
                node.expand({}, true, terminal_value);
                if (cfg_.use_transposition_table) {
                    transposition_table_[tt_key] = TranspositionEntry{
                        terminal_value, true, terminal_value, pending_from, {}, {}
            ...
            const auto encode_start = std::chrono::steady_clock::now();
            const EncodedState encoded = env.encode_state(pending_from);
            if (profile_ != nullptr) {
                profile_->encode_state.add(seconds_since(encode_start));
            }
            const auto build_start = std::chrono::steady_clock::now();
            std::vector<ActionEntry> entries = build_action_entries(env, encoded, dests, false, pending_from);
            if (profile_ != nullptr) {
                profile_->build_action_entries.add(seconds_since(build_start));
            }
            ...
            // Destination node children have pending_from=nullopt (source nodes)
            node.expand(child_specs, false, 0.0f, std::nullopt);
            ...
            if (cfg_.use_transposition_table) {
                ...
                transposition_table_[tt_key] = TranspositionEntry{
                    value, false, 0.0f, pending_from, child_specs, stored_entries
                };
            }
            if (profile_ != nullptr) {
                profile_->expand_node.add(seconds_since(expand_start));
        ...
        // ── Source node: expand with unique source coordinates + submit ──
        if (legal_semimoves.empty() && !can_submit) {
            const float terminal_value = no_legal_action_terminal_value();
            node.expand({}, true, terminal_value);
            if (action_entries_out != nullptr) {
        ...
        std::vector<Semimove> unique_sources;
        {
            std::set<std::tuple<int, int, int, int>> seen; // l, x, y, t
            for (const auto& sm : legal_semimoves) {
                auto key = std::make_tuple(sm.from.l(), sm.from.x(), sm.from.y(), sm.from.t());
                if (seen.insert(key).second) {
                    unique_sources.push_back(sm);
        ...
        node.expand(child_specs, false, 0.0f, std::nullopt);
        // Set pending_from for non-submit children (destination nodes)
        for (auto& child : node.children) {
            if (!child->action.is_submit) {
                child->pending_from = child->action.semimove;
            }
...
private:
    ...
    [[nodiscard]] GameResult play_game(int board_limit) {
        ...
        while (!env.done() && (!use_max_game_length || env.total_semimoves() < cfg_.max_game_length)) {
            ...
            const float urgency = static_cast<float>(boards_remaining);
            const float temperature = env.total_semimoves() < cfg_.temperature_threshold
                ? cfg_.temperature
                : cfg_.temperature_final;
            const auto encode_start = std::chrono::steady_clock::now();
            const EncodedState encoded = env.encode_state();  // training samples: NO pending_from marker
            profile_.encode_state.add(seconds_since(encode_start));
            const auto search_result = mcts_.select_action(env, urgency, temperature, pending_from);
            ...
            const int player = env.current_player();
            if (!pending_from.has_value()) {
                // ── Source node: record training sample and select source ──
                result.samples.push_back(GameResult::TrainingSample{
                    .encoded_state = encoded,
                    .action_entries = search_result.action_entries,
                    .policy_probs = search_result.policy_probs,
                    .urgency = urgency,
                    .value_target = 0.0f,
                    .player = player,
                    .pending_x = -1,
                ...
                if (search_result.action.is_submit) {
                    ...
                    result.move_history.push_back(MoveLogEntry{
                        player, true, "SUBMIT", search_result.root_value, env.board_count(),
                    });
                    if (outcome.has_value()) {
                    ...
                    // Source selected: remember it for the next halfstep
                    pending_from = search_result.action.semimove;
                }
            } else {
                // ── Destination node: complete the semimove and record ──
                Semimove full_sm = *pending_from;
                full_sm.to = search_result.action.semimove.to;
                result.samples.push_back(GameResult::TrainingSample{
                    .encoded_state = encoded,
                    .action_entries = search_result.action_entries,
                    .policy_probs = search_result.policy_probs,
                    ...
                    .value_target = 0.0f,
                    .player = player,
                    .pending_x = pending_from->from.x(),
                    .pending_y = pending_from->from.y(),
                    .pending_t = pending_from->from.t(),
                    .pending_l = pending_from->from.l(),
                });
                const auto format_start = std::chrono::steady_clock::now();
                const std::string move_text = env.format_move(full_sm);
                profile_.format_move.add(seconds_since(format_start));
                const auto apply_start = std::chrono::steady_clock::now();
                if (!env.apply_semimove(full_sm, true)) {
                ...
                result.move_history.push_back(MoveLogEntry{
                    player, false, move_text, search_result.root_value, env.board_count(),
                });
                pending_from.reset();
                if (env.done()) {
