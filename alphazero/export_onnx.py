...
class OnnxActionWrapper(nn.Module):
    ...
    def forward(
        ...
        value, submit_logit, _, board_out, _ = self.network(
            ...
            l_coords,
            t_coords,
            urgency,  # [B] — the network expects 1D urgency
            padding_mask=padding_mask,
            return_latent=True,
            side_to_move=side_to_move,
        )
        raw_logits = self.network.policy_head(board_out)  # [B, N, board_squares]
        A = action_board_indices.shape[0]

        # Compute both submit and non-submit logits for all actions,
        # then merge with torch.where (avoids data-dependent Python branches).
        submit_mask = action_is_submit.bool()

        # Submit logits: read submit_logit[state_idx] for each action
        sidx = action_state_indices.long().clamp(0, batch_size - 1)
        submit_logits = submit_logit.reshape(batch_size)[sidx]  # [A]

        # Non-submit logits: read raw_logits[state_idx, board_idx, from_sq] for each action
        N = board_out.shape[1]
        safe_sidx = sidx  # already clamped
        safe_bidx = action_board_indices.long().clamp(0, N - 1)
        safe_fsq = action_from_squares.long().clamp(0, self.network.cfg.board_squares - 1)
        non_submit_logits = raw_logits[safe_sidx, safe_bidx, safe_fsq]  # [A]

        logits = torch.where(submit_mask, submit_logits, non_submit_logits)

        # Keep unused ONNX inputs in the traced graph.
        logits = logits + (action_to_squares.float() * 0.0).sum()
        logits = logits + (action_delta_t * 0.0).sum()
        logits = logits + (action_delta_l * 0.0).sum()
...
def _build_dummy_inputs(
    ...
    side_to_move = torch.zeros((batch_size,), dtype=torch.long, device=device)
