...
# Sentinel action for submitting the turn (end current player's action).
# Used throughout halfstep_state and MCTS as a special action value.
...
class SemimoveEnv:
    ...
    def encode_state(self, urgency: float = 0.0,
                     pending_from: Optional[tuple[int, int, int, int]] = None) -> dict:
        """
        Encode current game state into network input tensors.

        Args:
            urgency: boards remaining until forced end (scalar).
            pending_from: if set, marks the selected piece's square in board_planes
                          by negating all 12 channels, so the network can distinguish
                          source-selection from destination-selection nodes.

        Returns dict with:
          'board_planes':      [N, C, S] float32 (12 channels: 6 white + 6 black)
          'l_coords':          [N] int64
          't_coords':          [N] int64
          'urgency':           [1] float32
        ...
        for i, (l, t, c, fen) in enumerate(boards_info):
            ...
            # White pieces (engine ch 0-5 -> our ch 0-5)
            board_planes[i, 0:6, :] = crop[0:6].reshape(6, self.board_squares)
            # Black pieces (engine ch 12-17 -> our ch 6-11)
        ...
        if pending_from is not None:
            ...
            for i, (l, t, c) in enumerate(board_keys):
                if l == pf[3] and t == pf[2] and c == player:
                    sq = pf[0] + pf[1] * self.board_side
                    if 0 <= sq < self.board_squares:
                        board_planes[i, :, sq] *= -1.0
                    break
