...
class NetworkConfig:
    ...
    # Input channels per board: 6 white + 6 black = 12 planes. Side-to-move
    # is encoded via the [CLS] token (all 0s for white, all 1s for black).
    ...
    d_ff: int = 256                     # feed-forward inner dim
    dropout: float = 0.1
...
class MCTSConfig:
    ...
    num_simulations: int = 800          # MCTS simulations per move
    ...
    # On a proportion p of turns, perform a full search with N=num_simulations;
    # on the remaining (1-p) turns, perform a fast search with n=fast_sims << N.
    # Only full-search turns are recorded for training. Fast searches skip Dirichlet
    # noise to maximize strength. This accelerates self-play by generating more
    # games per unit compute while maintaining high-quality training targets.
    use_playout_cap_randomization: bool = True
    playout_cap_randomization_p: float = 0.25       # proportion of full-search turns
    playout_cap_randomization_fast_sims: int = 50   # simulations for fast search (n)
...
class TrainConfig:
    ...
    epochs_per_iteration: int = 10      # number of passes over the buffer per iteration
    steps_per_epoch: int = 10           # gradient steps per epoch (each step = one batch)
