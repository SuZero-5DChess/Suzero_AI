...
"""
Self-play module for halfstep-level AlphaZero training.

Each self-play game:
  1. Rolls a random board_limit in [min_board_limit, max_board_limit]
  2. Plays halfstep by halfstep using MCTS until terminal
...
from .halfstep_state import HalfstepGame
...
@dataclass
class HalfstepRecord:
    """Single training sample from self-play."""

    board_planes: np.ndarray      # [N, 12, 16]
    l_coords: np.ndarray          # [N]
    t_coords: np.ndarray          # [N]
    urgency: float                # scalar
    padding_mask: np.ndarray      # [N] bool

    # Policy target at root halfstep level (source squares + submit)
    policy_target: np.ndarray         # [A] visit distribution
    ...
    # Side to move (0=white, 1=black) — used to set [CLS] token in the network
    player: int = 0
...
class GameRecord:
    ...
    samples: list[HalfstepRecord] = field(default_factory=list)
...
class SelfPlayWorker:
    ...
    @staticmethod
    def _build_root_action_arrays(root, board_keys: list, env) -> dict:
        """Build action metadata from root node's children (halfstep level)."""
        children = root.children
        n = len(children)
        board_idx = np.full(n, -1, dtype=np.int64)
        from_sq = np.zeros(n, dtype=np.int64)
        is_submit = np.zeros(n, dtype=bool)
        player = env.current_player

        for i, child in enumerate(children):
            if child.action == SUBMIT_ACTION:
                is_submit[i] = True
            else:
                src = child.action
                target_c = bool(player)
                bi = -1
                for j, (l, t, c) in enumerate(board_keys):
                    if l == src[3] and t == src[2] and c == target_c:
                        bi = j
                        break
                # Fallback: 忽略颜色匹配（与 mcts._find_board_index 一致）
                if bi < 0:
                    for j, (l, t, c) in enumerate(board_keys):
                        if l == src[3] and t == src[2]:
                            bi = j
                            break
                board_idx[i] = bi
                from_sq[i] = src[0] + src[1] * env.board_side

        return {"board_idx": board_idx, "from_sq": from_sq, "is_submit": is_submit}

    def play_game(self) -> GameRecord:
        """Play a single self-play game, halfstep by halfstep, collecting training samples."""
        record = GameRecord()
        ...
        while not game.done and (not use_max_game_length or game.total_semimoves < max_game_length):
            boards_remaining = max(0, board_limit - game.board_count)
            urgency = boards_remaining

            if game.total_semimoves < self.sp_cfg.temp_threshold:
                temperature = self.sp_cfg.temperature
            else:
                temperature = self.sp_cfg.temperature_final

            # ── Playout cap randomization (Wu 2020, arXiv:1902.10565) ──
            # On a proportion p of turns, run a full search (N=num_simulations)
            # with Dirichlet noise; on the remaining (1-p) turns, run a fast
            # search (n=fast_sims << N) without noise. Only full-search turns
            # are recorded as training samples, so the policy learns from
            # high-quality targets while the game generates more data per unit
            # compute.
            use_full_search = True
            if self.mcts_cfg.use_playout_cap_randomization:
                use_full_search = random.random() < self.mcts_cfg.playout_cap_randomization_p

            # MCTS search on current halfstep state
            mcts_state = game.make_mcts_state()
            root = mcts.search(mcts_state, board_limit=board_limit,
                               use_full_search=use_full_search)
            halfstep_action, policy_probs, root_value = mcts.select_halfstep(root, temperature=temperature)

            if halfstep_action is None:
                terminated_by_no_action = True
                break

            # Determine action type for move history (before step mutates state)
            player = game.current_player
            is_submit = (halfstep_action == SUBMIT_ACTION)
            is_source = (not is_submit and game.pending_from is None)

            # Record training sample only from full-search turns
            if use_full_search:
                encoded = env.encode_state(urgency=urgency, pending_from=game.pending_from)
                action_arrays = self._build_root_action_arrays(root, encoded["board_keys"], env)

                sample = HalfstepRecord(
                ...
                player_at_step.append(game.current_player)

            # Record move history (always, regardless of full/fast search)
            if is_submit:
                ...
                record.move_history.append(
                    MoveEntry(
                        ...
                        ext_move_str=str(halfstep_action),
                        mcts_value=root_value,
                        board_count=game.board_count,
                    )
                )
                ...
                sm = Semimove(
                    line_idx=game.pending_from[3],
                    from_pos=game.pending_from,
                    to_pos=halfstep_action,
                )
                record.move_history.append(
                    MoveEntry(
                        player=player,
            ...
            # Apply the halfstep (mutates env via game)
            game.step(halfstep_action)

            # After SUBMIT, check for game outcome
            if is_submit and game.done and game.env.outcome is not None:
                break
        ...
        try:
            ...
            record.pgn = env.game.show_pgn(_engine.SHOW_CAPTURE | _engine.SHOW_PROMOTION)
...
class CppOnnxSelfPlayWorker:
    ...
    def _load_games_from_binary(self, path: Path) -> list[GameRecord]:
        ...
        with path.open("rb") as handle:
            ...
            if version != self.DATA_VERSION and version != 3:
            ...
            for _ in range(num_games):
                ...
                for _ in range(num_samples):
                    ...
                    # pending_from coordinates (v4+, 4 int32, -1 sentinel for source nodes)
                    pending_x = pending_y = pending_t = pending_l = -1
                    if version >= 4:
                        pending_x, pending_y, pending_t, pending_l = _read_struct(handle, "<iiii")
                    ...
                    board_side = int(self.train_cfg.network.board_side)
                    ...
                    # Restore pending_from marker for destination nodes.
                    # C++ binary binarizes board_planes to uint8 {0,1}, losing the
                    # -1 marker. We store the selected piece's coordinates separately
                    # and reconstruct the marker here.
                    # l_coords are shifted by l_coord_shift in C++ encode_state;
                    # subtract it back to match raw L for comparison.
                    if pending_l != -1:
                        l_coord_shift = 5  # kDefaultLineShift, matches C++
                        for bi in range(num_boards):
                            raw_l = int(l_coords[bi]) - l_coord_shift
                            if raw_l == pending_l and int(t_coords[bi]) == pending_t:
                                sq = pending_x + pending_y * board_side
                                if 0 <= sq < board_squares:
                                    board_planes[bi, :, sq] = -1.0
                                break
...
class ReplayBuffer:
    ...
    def __init__(self, capacity: int):
        ...
        self.buffer: list[HalfstepRecord] = []
        self.position = 0

    def push(self, samples: list[HalfstepRecord]):
    ...
    def sample(self, batch_size: int) -> list[HalfstepRecord]:
...
def collate_samples(
    samples: list[HalfstepRecord],
    device: torch.device,
    max_boards: int | None = None,
) -> dict:
    """
    Collate a batch of HalfstepRecords into padded tensors.
    ...
    side_to_move = np.array([s.player for s in samples], dtype=np.int64)
