...
def compute_loss(network: AlphaZeroNetwork, batch: dict,
    ...
    for i in range(B):
        ...
        raw_logits_i = network.policy_head(board_out[i, :n_valid])  # [N, board_squares]
        logits_i = torch.empty(len(abi), device=value.device)
        non_submit = ~ais
        if non_submit.any():
            # Guard against -1 board_idx (should not happen after _build_root_action_arrays fix)
            valid_mask = abi[non_submit] >= 0
            if valid_mask.any():
                abi_valid = abi[non_submit][valid_mask]
                afs_valid = afs[non_submit][valid_mask]
                logits_i[non_submit.clone()] = -1e8  # default for invalid indices
                logits_i[non_submit.clone()][valid_mask] = raw_logits_i[abi_valid, afs_valid]
            else:
                logits_i[non_submit] = -1e8
        logits_i[ais] = submit_logit[i].squeeze(-1)
...
class Trainer:
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        # T_max = total gradient steps across all iterations
        self._total_steps = cfg.num_iterations * cfg.epochs_per_iteration * cfg.steps_per_epoch
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer,
            T_max=self._total_steps,
        )
        logger.info(f"LR scheduler T_max={self._total_steps} "
                     f"({cfg.num_iterations} iters × {cfg.epochs_per_iteration} epochs "
        ...
        self._current_terminal_reasons = {}
    ...
    def _setup_signal_handlers(self):
        """Set up SIGINT/SIGTERM handlers for graceful interruption."""
        def _handler(signum, frame):
            if self._interrupted:
                logger.warning("收到第二次中断信号，强制退出...")
                sys.exit(1)
            signame = "SIGINT" if signum == signal.SIGINT else "SIGTERM"
            logger.warning("收到 %s 信号，将在当前迭代完成后优雅退出...", signame)
            self._interrupted = True

        signal.signal(signal.SIGINT, _handler)
        signal.signal(signal.SIGTERM, _handler)

    def train(self, num_iterations: int = None, continuous: bool = False):
        """Run the full training loop.

        Args:
            num_iterations: Number of iterations per block. Falls back to cfg.num_iterations.
            continuous: If True, loop indefinitely until interrupted. Each block runs
                        num_iterations iterations, then starts a new block.
        """
        ...
        while not self._interrupted:
            ...
            label = f"block {continuous_round + 1}" if continuous_round > 0 else "initial"
            logger.info(f"Starting AlphaZero training: iterations {start_iter+1}..{end_iter} ({label})")
            ...
            if self._interrupted:
                logger.info("训练被中断，保存最终 checkpoint...")
                self._save_checkpoint(self.iteration + 1, final=True)
                logger.info("训练已中断，所有数据已保存。重新运行可自动恢复。")
                return

            # Final save of this block
            self._save_checkpoint(end_iter, final=True)
            logger.info(f"完成 {n_iter} 轮迭代 (total={self.iteration + n_iter})")

            if not continuous:
                logger.info("训练完成！（使用 --continuous 可保持持续训练）")
                return

            continuous_round += 1
            logger.info(f"\n{'='*60}")
            logger.info(f"持续训练模式：开始第 {continuous_round + 1} 轮 ({n_iter} 轮/轮)")
            logger.info(f"{'='*60}")
    ...
    def _self_play_phase(self):
        ...
        self._current_terminal_reasons = dict(reasons)
        self._current_outcomes = {
            'white_wins': sum(1 for o in outcomes if o > 0),
            'black_wins': sum(1 for o in outcomes if o < 0),
            'draws': sum(1 for o in outcomes if o == 0),
        }
        white_wins = self._current_outcomes['white_wins']
        black_wins = self._current_outcomes['black_wins']
        draws = self._current_outcomes['draws']
    ...
    def _training_phase(self) -> dict:
        ...
        Each epoch consists of steps_per_epoch gradient steps, each on an
        independently sampled batch.  When the buffer is small, the effective
        steps per epoch are decreased to avoid overfitting on a tiny dataset.
        """
        self.network.train()
        ...
        # Dynamic step adjustment: when the buffer is small, limit passes to
        # avoid repeatedly sampling the same data.
        ...
        if effective_steps < self.cfg.steps_per_epoch:
            logger.info(
                "  Training steps capped: %d steps/epoch (buffer=%d, batch_size=%d, requested=%d)",
                effective_steps, buffer_size, self.cfg.batch_size,
                self.cfg.steps_per_epoch,
            )
    ...
    def _log_metrics(self, iteration: int, metrics: dict, iter_time: float):
        ...
        entry = {
            ...
            'terminal_reasons': self._current_terminal_reasons,
...
def main():
    ...
    parser.add_argument('--continuous', action='store_true',
                        help="Run continuously until interrupted (Ctrl+C for graceful stop)")
    ...
    trainer.train(num_iterations=args.iterations, continuous=args.continuous)
