...
def test_network():
    ...
    def check_predict(net):
        ...
        with torch.no_grad():
            stm = torch.tensor([0], dtype=torch.long, device=device)
            v, sl, rl = net.forward(
                bp.unsqueeze(0),      # [1, N, PC, BS]
                lc.unsqueeze(0),      # [1, N]  — l_coords
                tc.unsqueeze(0),      # [1, N]  — t_coords
                urg,                  # [1]     — urgency, scalar per sample
                side_to_move=stm,
            )
            v = v.item()
            sl = sl.item()
            rl = rl.squeeze(0)        # [N, board_squares]
...
def test_mcts():
    ...
    def check_mcts():
        ...
        t0 = time.time()
        mcts_state = HalfstepState(env)
        root = mcts.search(mcts_state, board_limit=10)
        action, policy, value = mcts.select_halfstep(root, temperature=1.0)
...
def test_mcts_suffix_reuse():
    ...
    def run_case(net, reuse_suffixes: bool):
        ...
        try:
            t0 = time.time()
            with torch.no_grad():
                root = mcts.search(HalfstepState(env), board_limit=10)
            elapsed = time.time() - t0
        finally:
...
def test_mcts_transposition_table():
    ...
    def check_tt_cache():
        ...
        try:
            node1 = MCTSNode(state=HalfstepState(env))
            v1 = mcts._expand(node1, urgency=0.5)
            node2 = MCTSNode(state=HalfstepState(env))
            v2 = mcts._expand(node2, urgency=0.5)
        finally:
    ...
    def check_no_legal_action_is_loss():
        ...
        node = MCTSNode(state=HalfstepState(DummyEnv()))
        value = mcts._expand(node, urgency=0.5)
...
def test_loss():
    ...
    from .network import AlphaZeroNetwork
    from .self_play import HalfstepRecord, collate_samples
